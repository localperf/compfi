...
optimize = function (changes, yearly.target) { 
	...
	weights 	= weights[weights$weight >= 0.5,]
...
optimize (changes, yearly.target = .16)
head(prices)
plot.common(prices)
#------------------------------------------------------------------------------------------------------------------
...
folio = c("ANN", "TNH", "HE", "JWN", "BOH", "BMO", "ARMH", "ENB", "TRP", "EAT", "EWC", "CNDA", "EWS", "HNR", "DVY", "LQD", "ENY", "IYR", "JUNR", "POT", "ENB")

mitre = c("fagix", "fcnkx", "amanx", "ficdx", "fxsix", "fdikx")
syms = folio
start = as.Date(ISOdate(2011,1,1))
end = as.Date(ISOdate(2013,7,25))
setup(folio)
setup(mitre)
